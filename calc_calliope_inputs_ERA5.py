...
def calliope_inputs_country(country,hub_height):
    # Solar PV
    t2m_data,country_mask,lon,lat,dates = energy_model_functions.load_country_weather_data(country,ddir,'t2m_ERA5-1hr_2018.nc','t2m','time',0)
    ssrd_data,country_mask,lon,lat, dates_rad = energy_model_functions.load_country_weather_data(country,ddir,'ssrd_ERA5-1hr_2018.nc','ssrd','time',0)
    solar_pv_cf = energy_model_functions.solar_PV_model(t2m_data,ssrd_data,country_mask)

    # Demand
    t2m_daily_data,country_mask,lons,lats,dates_daily = energy_model_functions.load_country_weather_data(country,ddir,'t2m_ERA5-1hr_2018.nc','t2m','time',24)
